...
{
	string word1 = term1.GetWord(), word2 = term2.GetWord();
...
istream& operator>>(std::istream& in, Word& word) 
{
	...
	while (true) 
	{
		...
		if (IsValidCharacter(CurrentStreamCharacter) && !isspace(CurrentStreamCharacter) && isspace(in.peek()) || !in /* Checks if in.peek() returns eof since associativity is left to right*/) 
		{
...
void TruncateNonAlphaChars(Word& term) 
{
		...
		for (int i = 0; i < Phrase.size(); i++)
		{
			...
			if (IsValidCharacter(CurrentCharacter) && isalpha(CurrentCharacter) && IsAscii(CurrentCharacter))
			{
		...
		{
			char CurrentCharacter = Phrase.at(i);
			// Skips if invalid character character not within [-1, 255]
			if (IsValidCharacter(CurrentCharacter) && isalpha(CurrentCharacter))
			{
...
void InitializeList(BST<Word> list[], ifstream& file)
{
	...
	while (file)
	{
		...
		AlphabetIndex =  NextWord.GetWord() != "" && IsAscii(NextWord.GetWord().at(0))? tolower(NextWord.GetWord().at(0)) - 97 : -1;
